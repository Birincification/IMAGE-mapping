...
mkdir -p $out
echo "[INFO] [STAR] ["`date "+%Y/%m/%d-%H:%M:%S"`"] $STAR --genomeDir $STARIndex --readFilesIn $fw $rw --runThreadN $nthreads --outFileNamePrefix $out --readFilesCommand zcat --outFilterMismatchNmax 20 --outSAMstrandField intronMotif"
